...
replaceOnce(chat, '  return (', '  return (<>{<button type="button" className="uc-support-float" onClick={() => setDirectSupportOpen(true)} aria-label="پشتیبانی">پشتیبانی</button>}{directSupportOpen && <SupportChat onClose={() => setDirectSupportOpen(false)} />}', 'direct support launcher');
