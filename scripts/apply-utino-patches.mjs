...
replaceOnce(chat, '  return (<>{<button type="button" className="uc-support-float" onClick={() => setDirectSupportOpen(true)} aria-label="پشتیبانی">پشتیبانی</button>}{directSupportOpen && <SupportChat onClose={() => setDirectSupportOpen(false)} />}', '  return (', 'remove inline support launcher');
