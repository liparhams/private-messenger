...
replaceOnce(chat,
  ...
  'chat state');
replaceOnce(chat,
  'db.from("conversations").select("id,type,title,created_by,created_at")',
  'db.from("conversations").select("id,type,title,created_by,created_at,is_channel,description,channel_username,is_public")',
  'conversation fields');
replaceOnce(chat,
...
replaceOnce(chat,
  ...
  `<div className="uc-sidebar-tools uc-create-tools">
...
replaceOnce(chat,
  ...
  `{channelOpen && <Modal onClose={() => setChannelOpen(false)} title="ساخت کانال"><div className="uc-channel-form">
...
replaceOnce(auth,
  ...
  '        {registrationEnabled && <div style={{ marginTop: 16, textAlign: "center" }}><a href="/register/" style={{ color: "#60a5fa", fontWeight: 750, fontSize: 13, textDecoration: "none" }}>ساخت حساب جدید</a></div>}\n        <div style={{ marginTop: 12, textAlign: "center", color: colors.muted, fontSize: 10 }}>Username access · Messenger</div>',
  'registration link');

const admin = 'app/admin/page.js';
replaceOnce(admin,
  'server_error:"ارتباط با سرور برقرار نشد. اینترنت یا Supabase را بررسی کن."',
  'server_error:"ارتباط با سرور برقرار نشد. اینترنت یا Supabase را بررسی کن.","Failed to fetch":"ارتباط با سرور برقرار نشد. اتصال اینترنت یا Supabase را بررسی کن.","Load failed":"ارتباط با سرور برقرار نشد. اتصال اینترنت یا Supabase را بررسی کن."',
  'admin network error');
