const FA={
 ...
 invalid_group_username:"گروه نمی‌تواند شناسه کانال داشته باشد.",
 ...
 profile_create_failed:"ساخت پروفایل انجام نشد. حساب ایجادشده پاک‌سازی شد. دوباره تلاش کنید.",
 ...
 audit_log_failed:"ثبت گزارش مدیریت ناموفق بود. عملیات کامل تلقی نمی‌شود.",
 ...
 invalid_conversation_kind:"نوع گفت‌وگو نامعتبر است.",
};
const EN={
 username_exists:"That username is already in use.",invalid_username:"Username must be 3–20 characters using a-z, numbers, or _.",invalid_channel_username:"Channel username is invalid.",channel_username_exists:"That channel username is already in use.",username_required_for_public_channel:"A public channel needs a public username.",invalid_group_username:"A group cannot use a channel username.",invalid_display_name:"Display name must be 1–80 characters.",invalid_password:"Password must be 6–128 characters.",weak_password:"Password must be 6–128 characters.",registration_disabled:"Registration is currently disabled.",registration_check_failed:"We could not check registration status. Please try again.",not_authenticated:"Please sign in first.",unauthorized:"Your session is not valid. Please sign in again.",forbidden:"You do not have permission to do this.",permission_denied:"You do not have permission to do this.",not_allowed:"This action is not allowed for your account.",invalid_title:"The title is invalid.",invalid_description:"The description is too long.",public_conversation_not_found:"This public group or channel is no longer available.",invalid_invite:"This invite link is invalid or expired.",user_not_found:"User not found.",profile_create_failed:"Could not create the profile. The created account was cleaned up. Please try again.",profile_update_failed:"Could not update the profile.",user_create_failed:"Could not create the user. Check the information and try again.",user_delete_failed:"Could not delete the user.",password_update_failed:"Could not change the password.",ban_update_failed:"Could not update the ban status.",audit_log_failed:"The audit log could not be saved. The action is not considered complete.",invalid_message:"The message is invalid.",network:"Could not connect to the server. Please try again.",server_error:"A server error occurred. Please try again.",internal_error:"Something went wrong. Please try again.",method_not_allowed:"This operation is not supported.",invalid_conversation_kind:"The conversation type is invalid."};
export function mapError(error,locale="fa"){
 const code=typeof error==="string"?error:(error?.code||error?.message||"");
 const raw=String(code).toLowerCase();
 const dict=locale==="en"?EN:FA;
 if(raw.includes("failed to fetch")||raw.includes("networkerror")||raw.includes("load failed")||raw.includes("fetch failed"))return dict.network;
 const key=Object.keys(dict).find(k=>raw===k||raw.includes(k));
 if(key)return dict[key];
 if(raw.includes("23505")||raw.includes("duplicate key")||raw.includes("unique constraint"))return locale==="en"?"This value is already in use.":"این مقدار قبلاً استفاده شده است.";
 if(raw.includes("permission denied")||raw.includes("row-level security")||raw.includes("rls"))return dict.permission_denied;
 if(/^4\d\d$/.test(raw)||raw.includes("status 400"))return locale==="en"?"The request is not valid.":"درخواست معتبر نیست.";
 if(/^5\d\d$/.test(raw)||raw.includes("status 500"))return dict.server_error;
 return locale==="en"?"Something went wrong. Please try again.":"خطایی رخ داد. لطفاً دوباره تلاش کنید.";
}
