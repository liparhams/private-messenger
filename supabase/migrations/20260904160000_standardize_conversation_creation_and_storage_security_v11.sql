...
declare
  ...
  v_invite text;
  v_member_count integer := coalesce(cardinality(p_member_ids),0);
...
begin
  ...
  insert into public.conversations(
    type,title,created_by,is_channel,description,channel_username,is_public,discoverable,badge,invite_token
  ) values (
    'group',v_title,v_user,v_kind='channel',v_description,v_username,v_public,v_public,'none',case when v_public then null else v_invite end
  ...
  if v_member_count > 0 then
    insert into public.conversation_members(conversation_id,user_id,role,left_at)
    select v_id,x,'member',null from unnest(p_member_ids) as x;
  end if;
  ...
  if v_kind = 'channel' then
    ...
    if v_member_count > 0 then
      insert into public.channel_members(channel_id,user_id,role)
      select v_id,x,'member' from unnest(p_member_ids) as x;
    end if;
  end if;
...
create policy messages_insert_sender_v6 on public.messages
for insert to authenticated
with check (
  ...
  and (
    (message_type='text' and length(trim(coalesce(content,''))) between 1 and 4000 and file_url is null)
    or
    (message_type='file' and file_url is not null and (storage.foldername(file_url))[1]=(select auth.uid())::text and length(trim(coalesce(file_name,''))) between 1 and 120 and file_type is not null)
  )
  and ((receiver_id is null) or (sender_id<>receiver_id and exists(
    select 1 from public.conversation_members cm
    where cm.conversation_id=messages.conversation_id and cm.user_id=messages.receiver_id and cm.left_at is null
  )))
  and not exists(
    select 1 from public.conversations c
    where c.id=messages.conversation_id and c.is_channel and c.created_by<>(select auth.uid())
  )
);
