...
set search_path = ''
as $$
  select p.id,p.username,p.display_name,p.public_id,p.is_verified,p.verification,p.role
...
set search_path = ''
as $$
declare me uuid := (select auth.uid()); v_name text := trim(coalesce(new_display_name,''));
begin
  if me is null then raise exception 'not_authenticated'; end if;
  if length(v_name) < 1 or length(v_name) > 80 then raise exception 'invalid_display_name'; end if;
  update public.profiles set display_name=v_name where id=me;
...
begin
  select * into m from public.messages where id=message_uuid for update;
  if not found then raise exception 'message_not_found'; end if;
  if m.sender_id<>(select auth.uid()) and not private.is_admin() then raise exception 'permission_denied'; end if;
  update public.messages
    set content=null,file_url=null,file_name=null,file_type=null,message_type='text',deleted_at=coalesce(deleted_at,now()),deleted_by=(select auth.uid()),edited_at=null
