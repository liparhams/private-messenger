...
alter table public.profiles
  add constraint profiles_role_check check (role in ('user','admin','support'));
...
-- Set the existing account with username parham as the administrator.
update public.profiles
set role = 'admin'
where lower(username) = 'parham';

-- Admins can manage profiles. Passwords remain exclusively in Supabase Auth.
create policy "admins can manage profiles"
on public.profiles for all to authenticated
using (public.is_admin())
with check (public.is_admin());

-- Admins can manage messages without exposing them to anonymous users.
